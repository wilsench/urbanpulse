<x-layouts.admin title="Tambah Lokasi — Admin UrbanPulse">
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <form method="POST" action="{{ route('admin.locations.store') }}" class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4 text-sm">
            @csrf

            <div>
                <label class="block text-slate-700 font-bold mb-1">Nama Lokasi</label>
                <input type="text" name="name" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-300 text-slate-900 focus:outline-none focus:border-amber-600 min-h-[44px]">
            </div>

            <div>
                <label class="block text-slate-700 font-bold mb-1">Deskripsi</label>
                <textarea name="description" rows="3" class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-300 text-slate-900 focus:outline-none focus:border-amber-600"></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="any" name="latitude" value="-6.5971" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[44px]">
                </div>
                <div>
                    <label class="block text-slate-700 font-bold mb-1">Longitude</label>
                    <input type="number" step="any" name="longitude" value="106.7949" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[44px]">
                </div>
            </div>

            <div>
                <label class="block text-slate-700 font-bold mb-1">Alamat Lengkap</label>
                <input type="text" name="address" class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[44px]">
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label class="block text-slate-700 font-bold mb-1">Skor Hijau (0-100)</label>
                    <input type="number" name="green_score" value="85" min="0" max="100" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[44px]">
                </div>
                <div>
                    <label class="block text-slate-700 font-bold mb-1">Aksesibilitas (0-100)</label>
                    <input type="number" name="accessibility_score" value="85" min="0" max="100" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[44px]">
                </div>
                <div>
                    <label class="block text-slate-700 font-bold mb-1">Jalan Kaki (0-100)</label>
                    <input type="number" name="walking_score" value="85" min="0" max="100" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[44px]">
                </div>
            </div>

            <div class="flex items-center gap-2 pt-2">
                <input type="checkbox" name="bike_friendly" value="1" checked id="bf" class="rounded bg-slate-100 border-slate-300 text-amber-600">
