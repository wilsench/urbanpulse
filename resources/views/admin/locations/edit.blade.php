<x-layouts.admin title="Edit Lokasi — Admin UrbanPulse">
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="pb-4 border-b border-slate-200/80 flex justify-between items-center">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Edit Lokasi: {{ $location->name }}</h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Perbarui rincian informasi dan skor lokasi publik.</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.locations.update', $location) }}" class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-xs space-y-4 text-xs sm:text-sm">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-slate-700 font-semibold mb-1">Nama Lokasi</label>
                <input type="text" name="name" value="{{ old('name', $location->name) }}" required class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 focus:outline-none focus:border-emerald-600 min-h-[40px]">
            </div>

            <div>
                <label class="block text-slate-700 font-semibold mb-1">Deskripsi</label>
                <textarea name="description" rows="3" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 focus:outline-none focus:border-emerald-600">{{ old('description', $location->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="any" name="latitude" value="{{ old('latitude', $location->latitude) }}" required class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[40px]">
                </div>
                <div>
                    <label class="block text-slate-700 font-semibold mb-1">Longitude</label>
                    <input type="number" step="any" name="longitude" value="{{ old('longitude', $location->longitude) }}" required class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[40px]">
                </div>
            </div>

            <div>
                <label class="block text-slate-700 font-semibold mb-1">Alamat Lengkap</label>
                <input type="text" name="address" value="{{ old('address', $location->address) }}" class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[40px]">
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label class="block text-slate-700 font-semibold mb-1">Skor Hijau (0-100)</label>
                    <input type="number" name="green_score" value="{{ old('green_score', $location->green_score) }}" min="0" max="100" required class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[40px]">
                </div>
                <div>
                    <label class="block text-slate-700 font-semibold mb-1">Aksesibilitas (0-100)</label>
                    <input type="number" name="accessibility_score" value="{{ old('accessibility_score', $location->accessibility_score) }}" min="0" max="100" required class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[40px]">
                </div>
                <div>
                    <label class="block text-slate-700 font-semibold mb-1">Jalan Kaki (0-100)</label>
                    <input type="number" name="walking_score" value="{{ old('walking_score', $location->walking_score) }}" min="0" max="100" required class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 min-h-[40px]">
                </div>
            </div>

            <div class="flex items-center gap-2 pt-2">
                <input type="checkbox" name="bike_friendly" value="1" {{ $location->bike_friendly ? 'checked' : '' }} id="bf" class="rounded bg-slate-100 border-slate-300 text-emerald-600 focus:ring-emerald-500">
